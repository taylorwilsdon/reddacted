...
def write_report_header(target, url: str, overall_score: float, overall_sentiment: str, num_comments: int) -> None:
    """
...
def should_show_result(result: "AnalysisResult", pii_only: bool = False) -> bool:
    ...
    has_llm_pii = (
        result.llm_findings is not None and 
...
def write_summary_section(target, total_comments: int, sentiment_scores: List[float],
                          max_risk_score: float, riskiest_comment: str,
                          total_pii_comments: int = 0, total_llm_pii_comments: int = 0) -> None:
    """
    ...
    target.write(f"- Comments with PII Detected: {total_pii_comments} ({total_pii_comments/total_comments:.1%})\n")
    target.write(f"- Comments with LLM Privacy Risks: {total_llm_pii_comments} ({total_llm_pii_comments/total_comments:.1%})\n")
...
def generate_analysis_report(filename: str,
                             comments: List[Dict[str, Any]],
                             url: str,
                             results: List["AnalysisResult"],
                             overall_score: float,
                             overall_sentiment: str,
                             pii_only: bool = False) -> Dict[str, int]:
    """
    ...
    try:
        ...
        with open(filename, 'w') as target:
            ...
            for idx, result in enumerate(results, 1):
                ...
                if result.llm_risk_score > 0 or (result.llm_findings and result.llm_findings.get('has_pii', False)):
